...
class UseSavedShippingAddressResolver implements ResolverInterface
{
    ...
    public function resolve(Field $field, $context, ResolveInfo $info, array $value = null, array $args = null): array
    {
        ...
        $identity = $this->customerContext->identityForTool($context, $input['client_id'] ?? null, 'use_saved_shipping_address');
        return $this->idempotent->execute('use_saved_shipping_address', $input, $identity, function () use ($identity, $input): array {
            $address = $this->customers->ownedAddress($identity, (int)($input['address_id'] ?? 0));
            return $this->checkout->useCustomerAddress($identity, 'shipping', $address, $input['cart_id'] ?? null);
        });
    }
}
