...
class CloseConversationResolver implements ResolverInterface
{
    ...
    public function resolve(Field $field, $context, ResolveInfo $info, array $value = null, array $args = null)
    {
        ...
        $id = trim((string)($input['conversation_id'] ?? ''));
        if ($id === '') {
            throw new GraphQlInputException(__('Conversation id is required.'));
        }
        try {
            return $this->service->close($id, isset($input['client_id']) ? (string)$input['client_id'] : null, $this->customerContext->getCustomerId($context), 'graphql');
